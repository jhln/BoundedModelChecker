...
instance (Monad solver) => Functor (Tree solver) where
  --fmap f xs  =  xs >>= return . f
  fmap = liftM


instance (Monad solver) => Applicative (Tree solver) where
  pure = return
  (<*>) = ap

instance (Monad solver) => Monad (Tree solver) where
  return = Return
  (>>=) = extendTree

extendTree :: (Monad solver) => Tree solver a -> (a -> Tree solver b) -> Tree solver b
