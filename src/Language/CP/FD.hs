{-
 ...
 -  Tom Schrijvers
https://hackage.haskell.org/package/monadiccp-0.1/docs/src/Language-CP-FD.html
 -}
...
instance {-# OVERLAPS #-} Integral i => ToExpr i where
...
addArithmeticConstraint :: (ToExpr a, ToExpr b) =>
    ...
    (Domain -> Domain -> Domain) ->
    a -> b -> Expr
addArithmeticConstraint getZDomain getXDomain getYDomain xexpr yexpr = Expr $
  do
    x <- exprVar xexpr
    y <- exprVar yexpr
    xv <- lookup x
